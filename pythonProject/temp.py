...
def helper(dis):
    ...
    return desc, pre[0], med[0], die[0], wrkout
...
def getPrediction():
    return {
        "message": "this is prediction application"
    }
...
def predict():
    ...
    # Convert numpy array to list for precautions
    precautions = precautions.tolist()

    # Parse the string representation of the list for medications
    medications = eval(medications)
    rec_diet = eval(rec_diet)

    # Assuming other fields like dis_des, rec_diet, and workout are already in the correct format
